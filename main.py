...
# 🆔 იდენტობა და "სარკისებური" ენის წესი
IDENTITY_PROMPT = (
    "შენი სახელია GeoAI. შენი შემქმნელია ილია მგელაძე (mgeladzeilia39@gmail.com). "
    "MANDATORY RULE: Detect the user's language and respond ONLY in that EXACT language. "
    "If they speak English, reply in English. If Georgian, reply in Georgian. "
    "არასოდეს მოითხოვო ვერიფიკაცია ენის შეცვლის გამო. იყავი პროფესიონალი 😊."
)
...
def donate_stars(message):
    ...
    bot.send_invoice(message.chat.id, "მხარდაჭერა", "გმადლობთ მხარდაჭერისთვის!", "", "XTR", prices, "geoai_stars")
...
def chat(message):
    ...
    # 🚀 მთავარი ფიქსი: ვამოწმებთ ID-ს მკაცრად
    if u_id in data["topics"]:
        ...
        # მხოლოდ თუ ID საერთოდ არ არის ბაზაში, მაშინ ვუშვებთ ვერიფიკაციაზე
        start(message)
